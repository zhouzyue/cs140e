...
impl Gpio<Output> {
    ...
    pub fn set(&mut self) {
        self.registers.SET[self.pin as usize / 32].write(1 << (self.pin % 32))
    }

    /// Clears (turns off) the pin.
    pub fn clear(&mut self) {
        self.registers.CLR[self.pin as usize / 32].write(0)
    }
}
...
impl Gpio<Input> {
    ...
    pub fn level(&mut self) -> bool {
        self.registers.LEV[self.pin as usize / 32].has_mask(1 << (self.pin % 32))
    }
}
