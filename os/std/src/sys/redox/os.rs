...
pub fn env() -> Env {
    ...
    if let Ok(mut file) = super::fs::File::open("env:") {
...
pub fn getenv(key: &OsStr) -> io::Result<Option<OsString>> {
    if ! key.is_empty() {
        if let Ok(mut file) = super::fs::File::open(&("env:".to_owned() + key.to_str().unwrap())) {
            let mut string = String::new();
...
pub fn setenv(key: &OsStr, value: &OsStr) -> io::Result<()> {
    if ! key.is_empty() {
        let mut file = super::fs::File::create(&("env:".to_owned() + key.to_str().unwrap()))?;
